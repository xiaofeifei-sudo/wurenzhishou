 ...
 * @param route
 * @param homeRoute
 * @return {[(*&{icon})]|{meta: ({title}|*), icon: (*|string), name}[]}
 */
export const getBreadCrumbList = (route, homeRoute) => {
  ...
  console.log('route中的匹配', routeMatched);
...
export const getNewTagList = (list, newRoute) => {
  ...
  let newList = [...list];
  console.log('获取新Tags', newRoute);
  let index = -1;
  if (newRoute.name == 'shopLive') {
    index = newList.findIndex(item => item.query.shopId == query.shopId);
  } else {
    index = newList.findIndex(item => item.name === name);
  }
  if (index >= 0) {
    newList[index] = { name, path, meta, query };
  } else {
    newList.push({ name, path, meta, query });
  }
  return newList;
};
export const routeEqual = (route1, route2) => {
  // return route1.meta.title === route2.meta.title;
  if (route1.name == 'shopLive' || route2.name == 'shopLive') {
    return route1.query.shopId == route2.query.shopId;
  }
...
export const showTitle = (item, vm) => {
  ...
  if (useI18n) {
    ...
    if (item.name == 'shopLive' && item.query) {
      console.log('设置title', item);
      title = item.query.shopName
    } else {
