    ...
    >
      ...
      >
        ...
        <div :class="{'collapsed':collapsed}" class="logo-con">
          <img :src="maxLogo" style="width: 100%;height: 50px" key="max-logo" v-show="!collapsed"/>
          <img :src="minLogo" style="width: 100%;height: 50px" key="min-logo" v-show="collapsed"/>
        </div>
...
export default {
  ...
  watch: {
    ...
    $route (newRoute) {
      console.log('route改变了', newRoute);
      let { name, query, params, meta } = newRoute;
      // if (name == 'shopLive') {
      //   name = query.shopId;
      //   meta.title = query.shopName;
      // }
      this.addTag({
        route: {
          name,
  ...
  mounted () {
     ...
     */
    this.setTagNavList();
    console.log(routers, '总路由');
    // 初始化设置首页路由
    this.setHomeRoute(routers);
    this.setCollapsed();
    console.log('当前$route为', this.$route);
  ...
  methods: {
    ...
    handleSelectShop (item) {
      console.log('选择门店', item);
      this.updateCurrentShop(item);
      this.$router.push({
        name: 'shopLive',
        query: { shopId: item.id, shopName: item.store_name }
      });
    },
    initSideMenu () {
    ...
    async getShopData () {
      let that = this;
      let userId = this.$store.state.user.userLoginInfo.id;
      return new Promise((resolve) => {
        shopApi.getWorkBench({
          user_id: userId,
          store_id: this.$config.tenantId
        }).then((res) => {
          // console.log('控制台内容', res);
          if (res.status == 0) {
            let shopList = res.data.store_shop;
            that.updateWorkBench(res.data);
            // console.log('门店列表', shopList);
            that.shopList = shopList;
            that.updateShopList(shopList);
            resolve(true);
          } else {
    ...
    async getLostConn () {
      ...
      return new Promise((resolve) => {
        shopApi.getLostConn({
          ...
          console.log('断链设备', res);
          if (res.status == 0) {
            let shopList = res.data;
            if (shopList.leng > 0) {
              that.$Message.warning('您有设备掉线，请及时查看!');
              that.updateDisconnectShop(shopList);
            } else {
              that.updateDisconnectShop([]);
            }
          } else {
            that.updateDisconnectShop([]);
          }
    ...
    async buildMenuTree () {
      ...
      if (this.currentTopMenuName == 'Shop') {
        this.showShopList = true;
        await this.getShopData();
        await this.getLostConn();
      } else {
    ...
    recursion (children, parentMenu) {
      for (const router of children) {
        //验证权限
        // console.log('权限列表', this.$store.state.user.privilegeMenuKeyList);
        // if (this.$store.state.user.privilegeMenuKeyList.indexOf(router.name) === -1) {
        //   continue;
        // }
        // console.log(router.meta.hideInMenu);
